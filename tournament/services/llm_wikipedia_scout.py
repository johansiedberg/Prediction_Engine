...
_SYSTEM_PROMPT = (
    ...
    "CRITICAL REQUIREMENT FOR DISAMBIGUATION / GENERIC / HISTORY PAGES:\n"
    "If the article is a portal, disambiguation page, OR a generic tournament series/history page (e.g. 'FIFA World Cup', 'CONCACAF Gold Cup') rather than a specific upcoming edition (e.g. '2026 FIFA World Cup', '2025 CONCACAF Gold Cup'), you MUST set 'is_disambiguation': true and list the specific upcoming editions in the 'sub_tournaments' array.\n"
    "DO NOT extract historical performance tables, past hosts, or general team lists into the 'groups' array. Groups MUST only be extracted for a specific single tournament edition. DO NOT set 'is_disambiguation': true for multi-tier or multi-league tournaments (e.g. UEFA Nations League with League A, League B, League C). These MUST be treated as a single tournament.\n"
    "CRITICAL REQUIREMENT FOR SEEDING POTS, LEAGUES & PENDING DRAWS:\n"
    "If the tournament's groups have NOT been officially drawn yet (e.g. teams are merely listed in Seeding Pots like Pot 1, Pot 2, Pot 3, Pot 4, or allocated to broad leagues before the official draw), you MUST set 'draw_completed': false, 'groups': [], and 'fixtures': []. NEVER invent or construct fake pseudo-groups by taking one team from each pot row! If scheduled matchday date ranges exist, set 'scheduled_matchdays' to the number of matchdays, but leave 'fixtures' empty until confirmed drawn matches exist.\n"
    "CRITICAL REQUIREMENT FOR KNOCKOUT vs GROUP FIXTURES:\n"
    "Do NOT place future finals, semi-finals, or bracket placeholder boxes (e.g. 'Winner Match 1 vs Winner Match 2', 'Nations League Finals') into the 'fixtures' group match schedule. List them strictly in 'knockout_stages' and keep 'stage_or_group' clean without header fragments like 'Criteria for final overall ranking' or 'Tiebreakers'.\n"
    ...
    "IGNORE historical tables, past performance summaries, previous hosts, or generic participant lists from past editions. Only extract teams and groups for the specific current/upcoming tournament edition being described.\n"
