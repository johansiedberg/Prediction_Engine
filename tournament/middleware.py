        ...
        if port == '2029':
            if path == '/' or path == '/login/':
                if request.method == 'POST':
                    from tournament.views import engine_admin_login_view
                    return engine_admin_login_view(request)
                else:
                    from tournament.views import engine_admin_root_view
                    return engine_admin_root_view(request)
            elif path == '/logout/':
                if request.method == 'POST':
                    from tournament.views import engine_admin_logout_view
